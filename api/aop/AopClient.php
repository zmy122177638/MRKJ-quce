			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					...
					if (strpos($signData->signSourceData, "\\/") > 0) {
                        ...
                        $json=$signData->signSourceData;//浏览器无法回调再进行的操作
                        $arr=json_decode($json,true);

                        if($arr['msg']=='Success'){
                                $otn=mb_substr($arr['out_trade_no'],0,2);
                                if($otn=='HH'){
                                    header("location:http://120.24.82.134/zhiming/index.php/Home/Bzhh/return_url?shop={$arr['out_trade_no']}&appid=2016101302144443");
                                }else if($otn=='JP'){
                                    header("location:http://120.24.82.134/zhiming/index.php/Home/Bzjp/return_url?shop={$arr['out_trade_no']}&appid=2016101302144443");
                                }
                        }
                            header("location:http://120.24.82.134/zhiming/index.php/Home/Index/return_url?shop={$arr['out_trade_no']}&appid=2016101302144443");
